...
def reformat_weather(name, id, weather_info, daynow):
    ...
    for j in weather_pd.index.values:
        if j <= index0:
            weather_pd.loc[j, "date"] = (
                str(today)
                + str(" ")
                + str(weather_pd.loc[j, "od21"]).zfill(2)
                + str("h")
            )
        else:
            weather_pd.loc[j, "date"] = (
...
def figplot_weather(weather_sorted):
    ...
    plot_w = weather_sorted[
        (weather_sorted["area_name"] == plot_city)
        & weather_sorted["date"].str.contains(plot_date)
...
def job_suzhou():
    ...
    action = str("1")
    if action == str("1"):
        weather_result = Get_weatherinfo(suzhou_pd, savefile)
    elif action == str("2"):
